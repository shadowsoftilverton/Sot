...
// Returns TRUE on successful use of such a talent, FALSE otherwise.
int TalentMeleeAttack(object oIntruder = OBJECT_INVALID, object oSelf = OBJECT_SELF);
...
int TalentMeleeAttack(object oIntruder = OBJECT_INVALID, object oSelf = OBJECT_SELF)
{
    ...
    int bOkToWhirl = GetOKToWhirl(oSelf);
    int bHasFeat = GetHasFeat(FEAT_WHIRLWIND_ATTACK);
    int bPlayableRace = FALSE;
    if (GetIsPlayableRacialType(oSelf) || GetTag(oSelf) == "x2_hen_valen")
      bPlayableRace = TRUE;
       ...
       &&  bNumberofAttackers == TRUE)
    {
        ...
        ActionUseFeat(FEAT_WHIRLWIND_ATTACK, oSelf);
        return TRUE;
    }
    else
    // * Try using expertise
    ...
    {
        ClearActions(CLEAR_X0_I0_TALENT_MeleeAttack1);
        SetActionMode(oSelf, ACTION_MODE_EXPERTISE, TRUE);
    ...
    {
        ClearActions(CLEAR_X0_I0_TALENT_MeleeAttack1);
        SetActionMode(oSelf, ACTION_MODE_IMPROVED_EXPERTISE, TRUE);
        WrapperActionAttack(oTarget);
        return TRUE;
    }
    else
    if(nDiff < 10 && GetLocalInt(oSelf, "MeleeTalentCooldown") < 1)
    {
        ClearActions(CLEAR_X0_I0_TALENT_MeleeAttack1);
           ...
           && VerifyCombatMeleeTalent(tUse, oTarget))
        {
            ...
            bkTalentFilter(tUse, oTarget);
            SetLocalInt(oSelf, "MeleeTalentCooldown", 1);
            DelayCommand(18.0f, DeleteLocalInt(oSelf, "MeleeTalentCooldown"));
            return TRUE;
        }
        else
        {
...
//Original TalentMeleeAttack, commented out for testing.
/*
int TalentMeleeAttack(object oIntruder = OBJECT_INVALID)
{
    ...
    if(nDiff < 10 && GetLocalInt(OBJECT_SELF, "MeleeTalentCooldown") < 1)
    {
        ClearActions(CLEAR_X0_I0_TALENT_MeleeAttack1);
        // * this function will call the BK function
        EquipAppropriateWeapons(oTarget);
           ...
           && VerifyCombatMeleeTalent(tUse, oTarget))
        {
            ...
            // February 6 2003: Did not have a clear all actions before it
            bkTalentFilter(tUse, oTarget);
            SetLocalInt(OBJECT_SELF, "MeleeTalentCooldown", 1);
            DelayCommand(18.0f, DeleteLocalInt(OBJECT_SELF, "MeleeTalentCooldown"));
            return TRUE;
        }
        else
        {
