...
void RebukeUndead(int nTurnLevel, int nTurnHD, int nVermin, int nElemental, int nConstructs, int nOutsider, int nClassLevel) {
    ...
    while(GetIsObjectValid(oTarget) && nHDCount < nTurnHD && GetDistanceToObject(oTarget) <= 20.0)
    {
        ...
        {
            nHD = GetHitDice(oTarget) + GetTurnResistanceHD(oTarget) + GetLocalInt(oTarget, "UndeadSys_BolsterResistance"); //Invictus 2012-06-03
...
void main()
{
    ...
    if(GetHasFeat(1565,OBJECT_SELF))//Gift of the Divine
    {
        nTurnLevel=nTurnLevel+GetLevelByClass(50,OBJECT_SELF);
        nClassLevel=nClassLevel+GetLevelByClass(50,OBJECT_SELF);
    }
