...
void main() {
    ...
    if(nCtTargetAlign > 0)
        SendMessageToPC(OBJECT_SELF, "You sense the presence of chaos.");
    else {
        SendMessageToPC(OBJECT_SELF, "You sense no chaos nearby.");
        return;
    }
    ...
    if(nMxAura > 0) {
        ...
        if(nMxAura > GetHitDice(OBJECT_SELF) * 2 && nMxAura > 10 && GetAlignmentLawChaos(OBJECT_SELF) == ALIGNMENT_LAWFUL) {
            SendMessageToPC(OBJECT_SELF, "You are left reeling and stunned by the overwhelming chaos here!");
    ...
    while(GetIsObjectValid(oTarget)) {
        if(GetObjectType(oTarget) == OBJECT_TYPE_CREATURE) {
            ...
            if(!GetHasSpellEffect(SPELL_UNDETECTABLE_ALIGNMENT, oTarget) && !GetHasSpellEffect(SPELL_NONDETECTION, oTarget)) {
                ...
                SendMessageToPC(OBJECT_SELF, GetName(oMisTarget) +" is "+ sAuraMsg +" chaotic.");
            }
        }
