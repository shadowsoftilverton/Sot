...
//Modified petrification
//Lasts nPower in rounds on PC, and nPower in turns on NPC
//Kicks henchmen from party if petrified.
void DoPetrification(int nPower, object oSource, object oTarget, int nSpellID, int nFortSaveDC)
{
    ...
    if(!MySavingThrow(SAVING_THROW_FORT, oTarget, nFortSaveDC))
    {
        if (bIsPC == TRUE)
        {
            ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eLink, oTarget, RoundsToSeconds(nPower));
        }

        else
        {
            ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eLink, oTarget, TurnsToSeconds(nPower));
...
int spellsIsTarget(object oTarget, int nTargetType, object oSource)
{
    ...
    // If we have mastery of shapes, all of our offensive spells are always
    // selectively hostile.
    if(nTargetType == SPELL_TARGET_STANDARDHOSTILE &&
       GetHasFeat(FEAT_MASTERY_OF_SHAPES, oSource))
    {
