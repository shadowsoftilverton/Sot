...
// des_crft_spells.2da
//
// Or you can just add the conditions here to mitigate the amount of needless
// 2da edits for custom modules.
// - Foxtrot
//------------------------------------------------------------------------------
int X2CastOnItemWasAllowed(object oItem)
{
    int nID = GetSpellId();
    SendMessageToPC(GetFirstPC(), "Checking item. Spell ID: " + IntToString(nID));

    int bAllow = (Get2DAString(X2_CI_CRAFTING_SP_2DA, "CastOnItems", nID) == "1")
              || (GetStringRight(GetTag(oItem), 8) == "_pc_body" && (nID == SPELL_RAISE_DEAD || nID == SPELL_RESURRECTION));
    if (!bAllow)
    {
...
int X2PreSpellCastCode()
{
   ...
   //SendMessageToPC(oAveDemo,"Debug: spell include checked");
   //---------------------------------------------------------------------------
