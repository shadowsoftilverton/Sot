...
void spellsCure(int nDamage, int nMaxExtraDamage, int nMaximized, int vfx_impactHurt, int vfx_impactHeal, int nSpellID)
{
    ...
    int nExtraDamage = GetCasterLevel(oCaster); // * figure out the bonus damage
    ...
    if (nMetaMagic == METAMAGIC_MAXIMIZE)
    {
        ...
        if(GetIsPC(oCaster) && GetGameDifficulty() < GAME_DIFFICULTY_CORE_RULES)
        {
            nDamage = nDamage + nExtraDamage;
        }
    }
    ...
    if (GetRacialType(oTarget) != RACIAL_TYPE_UNDEAD)
    {
        ...
        //nHeal = nDamage;  -- what the fuck is this shit and why the fuck is it a thing
        //Set the heal effect
