...
void main()
{
   ...
   SetLocalObject(oTarget, "MountActionAttempter", oPC);
   ...
   { // SPELL/FEAT ID - switch to control horse actions
       ...
       { // Mount Menu Selection
           ...
           if(GetLocalInt(oTarget, "HorseParking") == 1 && GetLocalObject(oTarget, "HorseOwner") != GetLocalObject(oTarget, "MountActionAttempter"))
/*------*/ { //No stealing in Park Zone
                FloatingTextStringOnCreature("This horse is not yours and it is under watch.", oPC, FALSE);
                return;
           } //No stealing in Park Zone
           if (GetStringLength(GetLocalString(oTarget,"X3_HORSE_SCRIPT_MOUNT"))>0)
...
//----------------                                   Added second part by Hardcore UFO: No stealing horses in safekeeping areas
               { // PC Can mount it
                    if(GetLocalInt(oTarget, "HorseParking") == 1 && GetLocalObject(oTarget, "HorseOwner") != GetLocalObject(oTarget, "MountActionAttempter"))
                    {
                        FloatingTextStringOnCreature("This horse is not yours and it is under watch.", oPC, FALSE);
