...
int chooseTactics(object oIntruder)
{
    ...
    //Modified by Hardcore UFO to only count caster classes
    //nMagic = nMagic + GetHitDice(OBJECT_SELF);
    nMagic = nMagic + GetCasterLevel(OBJECT_SELF);
   ...
   // **************************************
    ...
    if (TalentMeleeAttack(oIntruder, OBJECT_SELF) == TRUE) return 99;      //Remove Object Self if KD cooldown needs to be reverted
...
void DetermineCombatRound(object oIntruder = OBJECT_INVALID, int nAI_Difficulty = 10)
{
    ...
    if(GetIsObjectValid(oIntruder))
    {
        ...
        //Commented out by Hardcore UFO - Universally picking the nearest enemy makes it inflexible.
        //Added clause for oNearEnemy to be the current atempted target (since previous clauses rule out
        //the caller still being attacking invalid ones at this point).
        //OnCombatRoundEnd will instead determine who this attackee is.

        //object oNearEnemy = GetNearestSeenEnemy();
        //DetermineCombatRound(oNearEnemy);

        //Added Block   Start
        object oNearEnemy = GetAttackTarget();
        if(!GetIsObjectValid(oNearEnemy))  {oNearEnemy = GetAttemptedAttackTarget();}
        else if(!GetIsObjectValid(oNearEnemy))  {oNearEnemy = GetAttemptedSpellTarget();}
        else if(!GetIsObjectValid(oNearEnemy))  {oNearEnemy = GetNearestSeenEnemy();}
