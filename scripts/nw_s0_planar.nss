...
//If targeted on an Elemental or Outsider, it will paralyze them (+2 on the DC).
...
void main()
{
    ...
    if(GetIsObjectValid(oTarget))
    {
        if(!GetIsReactionTypeFriendly(oTarget))
        {
            ...
            {
                //Make a will save
                if(!WillSave(oTarget, GetSpellSaveDC() + 2))
                {
                    //Apply the linked effect
                    DoPlanarHold(oTarget, GetSpellSaveDC() + 2, nDuration);
                }
            }
        }
    }
