...
const int HORSE_DEFAULT_SPEED_INCREASE= 40;
...
{ // PURPOSE: Change the movement speed based on that of the horse (wont work unless EffectMovementSpeedIncrease() bug is fixed)
    ...
    if (nSpeed>70) nSpeed=70; // upper limit
...
{ // PURPOSE: Spawn a horse with the specified information
    ...
    { // resref specified
        ...
        { // horse successfully created
            ...
            { // assign owner
                if(nHenchmen>0)//Ave 4/4/2012
                {
                    SendMessageToPC(oOwner,"You already have a horse!");
...
/*            else                                                 //Commented out by Hardcore UFO
            { // no post mount script                              //Overwrites custom scripts
                HORSE_SupportIncreaseSpeed(oRider,oHorse);         //All of which cover these parameters in a more SoT-adapted fashion
...
{ // PURPOSE: HorseChangeToDefault
    ...
    if(GetHasNonStandardAppearance(oCreature)) SetCreatureAppearanceType(oCreature,nDefApp); // Edit by Invictus
